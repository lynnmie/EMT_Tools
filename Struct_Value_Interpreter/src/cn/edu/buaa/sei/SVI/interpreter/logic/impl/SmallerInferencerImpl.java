...
import cn.edu.buaa.sei.SVI.interpreter.logic.SmallerInferencer;
...
public class SmallerInferencerImpl implements SmallerInferencer{

	@Override
	public Boolean interpret(LogicStruct input) throws Exception {
		if(input==null)
			throw new Exception("Null input is invalid");
		if(!(input instanceof Smaller))
			throw new Exception("Smaller required");
		
		Smaller op = (Smaller) input;
