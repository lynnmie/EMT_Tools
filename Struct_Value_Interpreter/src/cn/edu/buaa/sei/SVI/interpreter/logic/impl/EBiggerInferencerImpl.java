...
import cn.edu.buaa.sei.SVI.interpreter.logic.EBiggerInferencer;
...
public class EBiggerInferencerImpl implements EBiggerInferencer{

	@Override
	public Boolean interpret(LogicStruct input) throws Exception {
		if(input==null)
			throw new Exception("Null input is invalid");
		if(!(input instanceof EBigger))
			throw new Exception("EBigger required");
		
		EBigger op = (EBigger) input;
