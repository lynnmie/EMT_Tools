...
public class ESmallerInferencerImpl implements ESmallerInferencer{

	@Override
	public Boolean interpret(LogicStruct input) throws Exception {
		if(input==null)
			throw new Exception("Null input is invalid");
		if(!(input instanceof ESmaller))
			throw new Exception("ESmaller required");
		
		ESmaller op = (ESmaller) input;
