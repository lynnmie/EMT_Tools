...
public class BranchCoverageAssigner implements Assigner{
		...
		func.setBody(new FunctionExecutor(){
			...
			public void execute() throws Exception {
				...
				for(LClassObject cov:coverages){
					LDataObject stmt = (LDataObject) cov.get(cov.getType().getFeatureByName("brch"));
				...
				for(LClassObject cov:coverages){
					...
					LDataObject ts = (LDataObject) cov.get(cov.getType().getFeatureByName("tested_brch"));
				...
				System.out.println("brchs: "+((int)stmts)+"\t\t tested: "+tested);
	...
	public void back(LPackage model, IObjectWorld db, Struct top) throws Exception{
		...
		func.setBody(new FunctionExecutor(){
			...
			public void execute() throws Exception {
				...
				int tested = 0;
				for(LClassObject coverage:coverages){
					LDataObject ts = (LDataObject) coverage.get(coverage.getType().getFeatureByName("tested_brch"));
					if(ts==null||ts.integerVal()==null||ts.integerVal()<0)continue;
					tested+=ts.integerVal();
				}
